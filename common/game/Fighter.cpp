...
void Fighter::revive(Game* game) {
    ...
    for (int i = 0; i < PLAYER_NUM; i++) {
        ...
        if (game->players[i]-> isDead()) {
            ...
            // only want to increase HP of other hunters if they are dead
            if (!canAttack(game->players[i])) {
                // increase HP of players[i] TODO
        ...
        // queue this update to be send to other players TODO - do we need this??
            GameUpdate gameUpdate;
            gameUpdate.updateType = PLAYER_DAMAGE_TAKEN;
            gameUpdate.id = i;
            gameUpdate.damageTaken = attackDamage;
