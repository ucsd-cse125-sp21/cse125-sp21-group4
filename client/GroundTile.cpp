...
GroundTile::GroundTile(string fileName, char* textFile, glm::mat4* proj, glm::mat4* v, 
GLuint s, float scale, vector<glm::vec3> * transArr) {
	
	cout << "initing ground: " << textFile << endl;
	projection = proj;
	view = v;
	shader = s;
	translations = transArr;
	instanceCount = translations->size();
	model = glm::scale(glm::vec3(scale));

	// if path is NOT given at construction time, hasTexture will be false.
	if (loadTexture(textFile))
		cout << "loaded " << textFile << endl;
	else
		cout << "cannot load " << textFile << endl;
	...
	triangleSize = triangles.size(); // store for drawing later
...
void GroundTile::draw(glm::mat4 c) {
	...
	glDrawArraysInstanced(GL_TRIANGLES, 0, triangleSize * 3, instanceCount);
...
bool GroundTile::loadTexture(char* texturePath) {
